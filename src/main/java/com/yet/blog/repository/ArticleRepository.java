...
public interface ArticleRepository
    extends JpaRepository<ArticleEntity, Integer>, JpaSpecificationExecutor<ArticleEntity> {
  ...
  @Query(
      value =
          "SELECT COUNT (DISTINCT a.id) "
              ...
              + "AND type = :#{#conditionVO.type} AND tat.tag_id = :#{#conditionVO.tagId}",
      nativeQuery = true)
  Integer countArticleBacks(ConditionVO conditionVO);

  @Query(
      value =
          "SELECT a.id,article_cover,article_title,type,is_top,a.is_delete,a.status,a.create_time,category_name,t.id AS tag_id,t.tag_name \n"
